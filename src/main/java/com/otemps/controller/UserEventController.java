...
public class UserEventController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        sortCombo.getItems().addAll("Places (Croissant)", "Places (Décroissant)", "Date (Plus récent)");
    ...
    private void handleSortChange() {
        ...
        if (selected.contains("Places (Croissant)")) {
            currentSort = "nb_places";
            currentDir = "ASC";
        } else if (selected.contains("Places (Décroissant)")) {
            currentSort = "nb_places";
            currentDir = "DESC";
        } else {
    ...
    private void loadEvents(String search) {
        ...
        if (search.isEmpty()) {
            ...
            pageInfoLabel.setText("Résultats");
    ...
    @FXML private void handleNextPage() { currentPage++; loadEvents(""); }
    @FXML private void handlePrevPage() { currentPage--; loadEvents(""); }
    ...
    private VBox createEventUserCard(Event event) {
        ...
        Label date = new Label("📅 " + formatter.format(event.getDateDebut()));
        ...
        // Section News Météo
        Label weatherLabel = new Label("⛅ Météo : Vérification en cours...");
        ...
        // Récupération asynchrone OpenWeather (wttr.in)
        new Thread(() -> {
            try {
                // Utiliser la première partie du lieu (souvent la ville) pour la météo
                String place = event.getLieu().split(",")[0].trim().replace(" ", "+");
                // use format=1 to get generic weather without using % variable which causes URI exception
                ...
                if (res.statusCode() == 200) {
                    ...
                    // Extraire la température avec une regex (ex: pour "⛅️ +12°C" ou "🌧 9°C")
                    int temperature = 999;
                    ...
                    boolean isRaining = weather.toLowerCase().contains("rain") || weather.toLowerCase().contains("shower") 
                                     || weather.toLowerCase().contains("pluie") || weather.toLowerCase().contains("storm")
                                     || weather.toLowerCase().contains("drizzle") || weather.contains("🌧") || weather.contains("⛈");
                    
                    javafx.application.Platform.runLater(() -> {
                        if (isRaining) {
                            weatherLabel.setText("⚠️ Météo : " + weather + " - ÉVÉNEMENT ANNULÉ ! 🌧️");
                            ...
                            weatherLabel.setText("❄️ Météo : " + weather + " - TROP FROID (< 14°C)");
                            weatherLabel.setStyle("-fx-text-fill: #3b82f6; -fx-font-weight: bold; -fx-font-size: 11px;");
                            btn.setDisable(true);
                            btn.setText("Fermé (Météo)");
                        } else {
                            weatherLabel.setText("📰 News Météo : " + weather);
                            weatherLabel.setStyle("-fx-text-fill: #0ea5e9; -fx-font-weight: bold; -fx-font-size: 11px;");
                        }
                    });
                }
            } catch (Exception ex) {
                javafx.application.Platform.runLater(() -> weatherLabel.setText("📰 News Météo : Indisponible."));
            }
        }).start();
        return card;
    }
    ...
    private void showEventDetails(Event event) {
        try {
            ...
            // Refresh main list to update available places
            loadEvents(searchField.getText());
        } catch (Exception e) {
            e.printStackTrace();
            new Alert(Alert.AlertType.ERROR, "Erreur lors de l'ouverture des détails : " + e.getMessage()).show();
        }
    }


    @FXML private void handleGoToParticipations() {

        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/otemps/views/UserParticipations.fxml"));
    ...
    @FXML private void handleSwitchToAdmin() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/otemps/views/AdminDashboard.fxml"));
            Parent root = loader.load();
            Stage stage = (Stage) eventFlowPane.getScene().getWindow();
            stage.getScene().setRoot(root);
        } catch (Exception e) { e.printStackTrace(); }
    ...
    @FXML private void handleOpenGlobalMap() {
        Dialog<Void> dialog = new Dialog<>();
        dialog.setTitle("Où se trouvent nos événements ?");
        dialog.getDialogPane().getButtonTypes().add(ButtonType.CLOSE);
        
        WebView webView = new WebView();
        webView.setPrefSize(800, 600);
        
        java.util.List<Event> events = eventService.findAll();
        ...
        for (int i = 0; i < events.size(); i++) {
            ...
            jsonEvents.append("{")
                      ...
                      .append("}");
            if (i < events.size() - 1) jsonEvents.append(",");
        }
        jsonEvents.append("]");
        ...
        String mapHtml = "<!DOCTYPE html>\n" +
                         ...
                         "    <div id=\"map\"></div>\n" +
                         "    <script>\n" +
                         "        var events = " + jsonEvents.toString() + ";\n" +
                         "        var map = L.map('map').setView([48.8566, 2.3522], 6); // Centré sur l'Europe par défaut\n" +
    ...
    @FXML private void handleOpenCalendar() {
        Dialog<Void> dialog = new Dialog<>();
        dialog.setTitle("Calendrier des Événements");
        dialog.getDialogPane().getButtonTypes().add(ButtonType.CLOSE);
        ...
        Runnable updateCalendar = () -> {
            ...
            for (int day = 1; day <= daysInMonth; day++) {
                ...
                if (!eventsToday.isEmpty()) {
                    ...
                    Label evLabel = new Label(eventsToday.size() + " évèn.");
                    ...
                    dayCell.setOnMouseClicked(e -> {
                        if (eventsToday.size() > 0) {
                            showEventDetails(eventsToday.get(0));
                        }
                    });
